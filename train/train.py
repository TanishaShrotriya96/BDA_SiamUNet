...
config = {'labels_dmg': [0, 1, 2, 3, 4],
          ...
          'weights_loss': [0, 0, 1],
          'mode': 'dmg',
          'init_learning_rate': 0.005,#dmg: 0.005, #UNet: 0.01,           
          'device': 'cuda',
          'epochs': 150,
          'batch_size': 5,
          'num_chips_to_viz': 5,
          ...
          'starting_checkpoint_path': './outputs/train_UNet/checkpoints/checkpoint_epoch93_2022-05-06-21-41-03.pth.tar'}
...
def main():
    ...
    val_loader = DataLoader(xBD_val, batch_size=config['batch_size'], shuffle=False)
    ...
    # set up directories    
    checkpoint_dir = config['out_dir']+'/'+config['experiment_name']+'/'+'checkpoints'
    os.makedirs(checkpoint_dir, exist_ok=True)
    
    logger_dir = config['out_dir']+'/'+config['experiment_name']+'/'+'logs'
    os.makedirs(logger_dir, exist_ok=True)
    
    evals_dir = config['out_dir']+'/'+config['experiment_name']+'/'+'evals'
    os.makedirs(evals_dir, exist_ok=True)

    config_dir = config['out_dir']+'/'+config['experiment_name']+'/'+'configs'
    ...
    while (epoch <= epochs):
        ...
        model, optimizer, step_tr, confusion_mtrx_df_tr_dmg, confusion_mtrx_df_tr_bld = train(train_loader, model, criterion_seg_1, criterion_seg_2, criterion_damage, optimizer, epochs, epoch, step_tr, logger_train, logger_val, sample_train_ids, sample_val_ids, device,config['batch_size'])
        ...
        print("eval_results_tr_dmg: \n",eval_results_tr_dmg)
        eval_results_tr_dmg.to_csv("eval_results_tr_dmg.csv")
        ...
        print("tr_dmg_harmonic_mean: ", f1_harmonic_mean)
        ...
        print("eval_results_tr_bld: \n",eval_results_tr_bld)
        eval_results_tr_bld.to_csv("eval_results_tr_bld.csv")

        ####### validation
        logging.info(f'Model validation for epoch {epoch}/{epochs}')
        eval_start_time = datetime.now()
        confusion_mtrx_df_val_dmg, confusion_mtrx_df_val_bld, losses_val = validation(val_loader, model, criterion_seg_1, criterion_seg_2, criterion_damage, epochs, epoch, logger_val,config['batch_size'])
        ...
        print("eval_results_val_dmg: \n",eval_results_val_dmg)
        eval_results_val_dmg.to_csv("eval_results_val_dmg.csv")
        print("val_dmg_harmonic_mean_f1: \n",f1_harmonic_mean)
        ...
        print("eval_results_val_bld: \n",eval_results_val_bld)
        eval_results_val_bld.to_csv("eval_results_val_bld.csv")
...
def train(loader, model, criterion_seg_1, criterion_seg_2, criterion_damage, optimizer, epochs, epoch, step_tr, logger_train, logger_val, sample_train_ids, sample_val_ids, device, batch_size):
    """
    ...
    for batch_idx, data in enumerate(tqdm(loader)): 
        ...
        y_cls = data['damage_mask']

        # reshaping labels for multi-label classification
        # incorrect approach for multi-label, do this for multi-class
        y_cls_reshape = torch.reshape(y_cls,(batch_size,1,256,256)).tile((1,5,1,1)).to(torch.float32).numpy()
        for labels in y_cls_reshape:
            i=0
            for label in labels:
                label[label==i]=-1 #since both 1 and 0 are classes.
                label[label!=-1]=0
                label[label==-1]=1
                i=i+1
        y_cls_reshape=torch.from_numpy(y_cls_reshape).to(device=device)
        ...
        # calculate multi-label loss with cross-entropy
        
        # weights_damage_tf = torch.FloatTensor([1, 35, 70, 150, 120])
        # damage_loss = 0
        # for pred,label in zip(scores[2],y_cls.to(torch.float32)):
        #     sample_damage=0
        #     for cls,w in zip(pred,weights_damage_tf):
        #         sample_damage+=criterion_damage(cls,label)*w
        #     sample_damage = sample_damage/376
        #     damage_loss+=sample_damage
        # damage_loss=damage_loss/batch_size
        # a = damage_loss
        a=weights_loss[2]*criterion_damage(scores[2], y_cls)
        loss = weights_loss[0]*criterion_seg_1(scores[0], y_seg) + weights_loss[1]*criterion_seg_2(scores[1], y_seg) + a
...
def validation(loader, model, criterion_seg_1, criterion_seg_2, criterion_damage, epochs, epoch, logger_val,batch_size):
...
def compute_eval_metrics(epoch, labels_set, confusion_mtrx_df, eval_results):
    for cls in labels_set: 
        ...
        f1 = 0
        accuracy = 0
        tp = confusion_mtrx_df.loc[class_idx,'true_pos'].sum() 
        tn = confusion_mtrx_df.loc[class_idx,'true_neg'].sum()
        fp = confusion_mtrx_df.loc[class_idx,'false_pos'].sum()
        fn = confusion_mtrx_df.loc[class_idx,'false_neg'].sum()
        total = confusion_mtrx_df.loc[class_idx,'total_pixels'].sum()
        if tp!=0 or fp!=0:
            precision = tp/(tp + fp)
        if tp!=0 or fn!=0:
            recall = tp/(tp + fn)
...
def freeze_model_param(model):
    if type(model).__name__=='SiamUnet':
        for i in [0, 3]:
    ...
    if type(model).__name__ == 'SiamNet':
        
        for j in ['0','1','2','3','4','5','6','7']:
            each = model.downs._modules[j]
            # conv1
            each[0].weight.requires_grad = False
            # conv2
            each[3].weight.requires_grad = False
            # bias1
            each[1].weight.requires_grad = False
            # bias2
            each[4].weight.requires_grad = False
            each[1].bias.requires_grad = False
            each[4].bias.requires_grad = False

        for j in ['1','3','5','7','9','11','13','15']:
            each = model.ups._modules[j]
            # conv1
            each[0].weight.requires_grad = False
            # conv2
            each[3].weight.requires_grad = False
        ...
        for j in ['0','2','4','6','8','10','12','14']:
            each = model.ups._modules[j]
            each.weight.requires_grad = False
            each.bias.requires_grad = False

        model.conv_s.weight.requires_grad = False
        model.conv_s.bias.requires_grad = False
        model.bottleneck.weight.requires_grad = False
        model.bottleneck.bias.requires_grad = False

        model.nonlocal_.g._modules['0'].weight.requires_grad = False
        model.nonlocal_.g._modules['0'].bias.requires_grad = False
        model.nonlocal_.W._modules['0'].weight.requires_grad = False
        model.nonlocal_.W._modules['0'].bias.requires_grad = False
        model.nonlocal_.W._modules['1'].weight.requires_grad = False
        model.nonlocal_.W._modules['1'].bias.requires_grad = False
        model.nonlocal_.theta.weight.requires_grad = False
        model.nonlocal_.theta.bias.requires_grad = False
        model.nonlocal_.phi._modules['0'].weight.requires_grad = False
        model.nonlocal_.phi._modules['0'].bias.requires_grad = False
...
def reinitialize_Siamese(model):
    ...
    # 8 layered encoder-decoder
    if type(model).__name__=='SiamNet':

        # convTranspose
        torch.nn.init.xavier_uniform_(model.siam_bottleneck.weight)
        model.siam_bottleneck.bias.data.fill_(0.01)
        # conv2d
        torch.nn.init.xavier_uniform_(model.conv_c.weight)
        model.conv_c.bias.data.fill_(0.01)
        
        for j in ['0','2','4','6','8','10','12']:
            torch.nn.init.xavier_uniform_(model.siamups._modules[j].weight)
            model.siamups._modules[j].bias.data.fill_(0.01)

        # SiamNet
        model.siam_bottleneck_conv.apply(init_weights)

        for j in ['1','3','5','7','9','11','13']:
            model.siamups._modules[j].apply(init_weights)
